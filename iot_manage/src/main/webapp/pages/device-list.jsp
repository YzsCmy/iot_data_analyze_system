...
<body class="hold-transition skin-blue sidebar-mini">
				...
				<div class="box box-primary">
						...
						<div class="table-box">
							...
							<div class="box-tools pull-right">
								<div class="has-feedback">
									<input type="text" class="form-control input-sm"
										placeholder="搜索" id="q-device"> <span
							...
							<table id="dataList"
								...
								<tbody>
									...
									<c:forEach items="${pageInfo.list}" var="device">
										<tr>
											<td><input name="selectck" type="checkbox" value="${device.id}"></td>
											<td>${device.id }</td>
											<td>${device.name }</td>
											<td>${device.typeName }</td>
											<td>${device.pointName }</td>
											<td>${device.unitName }</td>
											<td>${device.statusStr }</td>
											<%--<security:authorize access="hasRole('ROOT')">--%>
											<%--<td>${user.unitName }</td>--%>
											<%--</security:authorize>--%>
											<td class="text-center">
												<a href="${pageContext.request.contextPath}/deviceData/findAll.do?deviceId=${device.id}" class="btn bg-olive btn-xs">详情</a>
											</td>
										</tr>
									</c:forEach>
								</tbody>
					...
					<div class="box-footer">
						...
						<div class="box-tools pull-right">
							<ul class="pagination">
								<li>
									<a href="${pageContext.request.contextPath}/device/findAll.do?page=1&size=${pageInfo.pageSize}&name=${query}" aria-label="Previous">首页</a>
								</li>
								<li id="preitem"><a href="${pageContext.request.contextPath}/device/findAll.do?page=${pageInfo.pageNum-1}&size=${pageInfo.pageSize}&name=${query}">上一页</a></li>
								<c:forEach begin="${pageInfo.pageNum-2<1?1:pageInfo.pageNum-2}" end="${pageInfo.pageNum+2>pageInfo.pages?pageInfo.pages:pageInfo.pageNum+2}"  var="pageNum">
									<li name="pageitem"><a href="${pageContext.request.contextPath}/device/findAll.do?page=${pageNum}&size=${pageInfo.pageSize}&name=${query}">${pageNum}</a></li>
								</c:forEach>
								<li id="nextitem"><a href="${pageContext.request.contextPath}/device/findAll.do?page=${pageInfo.pageNum+1}&size=${pageInfo.pageSize}&name=${query}">下一页</a></li>
								<li>
									<a href="${pageContext.request.contextPath}/device/findAll.do?page=${pageInfo.pages}&size=${pageInfo.pageSize}&name=${query}" aria-label="Next">尾页</a>
								</li>
							</ul>
						</div>
	...
	<script>
        ...
        $(function () {
            $('#q-device').bind('keypress', function(event) {
                if (event.keyCode == "13") {
                    event.preventDefault();
                    //回车执行查询
                    var query = $('#q-device').val();
                    // alert(query)
                    //向服务器发送请求
                    location.href = "${pageContext.request.contextPath}/device/findAll.do?name="+query;
                }
            });
        })
        $(function () {
        ...
        function editDevice() {
            ...
            if(i==0){
                ...
                location.href='${pageContext.request.contextPath}/device/edit.do?id='+id;
        ...
        function changePageSize() {
            ...
            //向服务器发送请求，改变没页显示条数
            location.href = "${pageContext.request.contextPath}/device/findAll.do?page=1&size="
                + pageSize+"&name=${query}";
        }
