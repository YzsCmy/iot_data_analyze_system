...
<body class="hold-transition skin-blue sidebar-mini">
				...
				<div class="box box-primary">
							...
							<table id="dataList"
								...
								<tbody>
									...
									<c:forEach items="${pageInfo.list}" var="user">
										<tr>
											<td><input name="selectck" type="checkbox" value="${user.id}"></td>
											<td>${user.id }</td>
											<td>${user.username }</td>
											<td>${user.phoneNum }</td>
					...
					<div class="box-footer">
						...
						<div class="box-tools pull-right">
							<ul class="pagination">
								<li>
									<a href="${pageContext.request.contextPath}/user/findAll.do?page=1&size=${pageInfo.pageSize}&name=${query}" aria-label="Previous">首页</a>
								</li>
								<li id="preitem"><a href="${pageContext.request.contextPath}/user/findAll.do?page=${pageInfo.pageNum-1}&size=${pageInfo.pageSize}&name=${query}">上一页</a></li>
								<c:forEach begin="${pageInfo.pageNum-2<1?1:pageInfo.pageNum-2}" end="${pageInfo.pageNum+2>pageInfo.pages?pageInfo.pages:pageInfo.pageNum+2}"  var="pageNum">
									<li name="pageitem"><a href="${pageContext.request.contextPath}/user/findAll.do?page=${pageNum}&size=${pageInfo.pageSize}&name=${query}">${pageNum}</a></li>
								</c:forEach>
								<li id="nextitem"><a href="${pageContext.request.contextPath}/user/findAll.do?page=${pageInfo.pageNum+1}&size=${pageInfo.pageSize}&name=${query}">下一页</a></li>
								<li>
									<a href="${pageContext.request.contextPath}/user/findAll.do?page=${pageInfo.pages}&size=${pageInfo.pageSize}&name=${query}" aria-label="Next">尾页</a>
								</li>
							</ul>
						</div>
	...
	<script>
        ...
        $(function () {
            $('#q-user').bind('keypress', function(event) {
                if (event.keyCode == "13") {
                    event.preventDefault();
                    //回车执行查询
                    var query = $('#q-user').val();
                    // alert(query)
                    //向服务器发送请求
                    location.href = "${pageContext.request.contextPath}/user/findAll.do?name="+query;
                }
            });
        })
        $(function () {
        ...
        function editUser() {
            ...
            if(i==0){
                ...
                location.href='${pageContext.request.contextPath}/user/edit.do?id='+id;
